...
run_docker_tests() {
    ...
    if docker compose -f docker-compose.yml --profile api-test run --rm api-test /app/test_runner.sh $extra_args; then
...
docker compose -f docker-compose.yml --profile api-test up -d db-test
...
case $TEST_SUITE in
    ...
    "build-only")
        ...
        if docker compose -f docker-compose.yml --profile api-test build api-test; then
    ...
    "shell")
        ...
        docker compose -f docker-compose.yml --profile api-test run --rm api-test sh
        ;;
    "clean")
        echo "Cleaning up test containers and volumes..."
        docker compose -f docker-compose.yml --profile api-test down -v --remove-orphans
...
if [ "$TEST_SUITE" != "shell" ] && [ "$TEST_SUITE" != "build-only" ] && [ "$TEST_SUITE" != "clean" ]; then
    ...
    docker compose -f docker-compose.yml --profile api-test down
fi
